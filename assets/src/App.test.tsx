				...
				<div key={trigger.id} data-testid={`trigger-${trigger.id}`}>
					...
					<button
						onClick={() => {
							if (
								window.confirm(
									`Are you sure you want to delete the "${trigger.name}" trigger? This action cannot be undone.`
								)
							) {
					...
					<button onClick={() => onSendTest(trigger)}>
						Send Test
					</button>
				</div>
			))}
...
describe('App Component', () => {
	...
	describe('Initial Rendering', () => {
		it('renders the integration list by default', () => {
			...
			expect(
				screen.queryByTestId('integration-detail')
	...
	describe('Navigation', () => {
		it('navigates to integration detail when clicking an integration', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByTestId('integration-detail')
				).toBeInTheDocument();
				expect(screen.getByText('WooCommerce')).toBeInTheDocument();
				expect(
					screen.queryByTestId('integration-list')
				).not.toBeInTheDocument();
			});
		});
		...
		it('navigates back to integration list when clicking back', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByTestId('integration-list')
				).toBeInTheDocument();
				expect(
					screen.queryByTestId('integration-detail')
				).not.toBeInTheDocument();
